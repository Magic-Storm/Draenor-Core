...
void ObjectMgr::LoadCreatureTemplatesDifficulties()
{
	...
	if (!l_Result)
	{
		TC_LOG_INFO("server.loading", ">> Loaded 0 creature template difficulties definitions. DB table `creature_template_difficulty` is empty.");
		return;
	}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u creature difficulties definitions in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadCreatureTemplateAddons()
{
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u creature template addons in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::CheckCreatureTemplate(CreatureTemplate const* cInfo)
{
	...
	for (uint32 diff = 0; diff < Difficulty::MAX_DIFFICULTY - 1 && ok; ++diff)
	{
		...
		{
			TC_LOG_ERROR("sql.sql", "Creature (Entry: %u) has different `npcflag2` in difficulty %u mode (Entry: %u).", cInfo->Entry, diff + 1, cInfo->DifficultyEntry[diff]);
			continue;
		}
	...
	{
		TC_LOG_ERROR("sql.sql", "Table `creature_template` lists creature (Entry: %u) with `exp_req` %u. Ignored and set to 0.", cInfo->Entry, cInfo->RequiredExpansion);
...
void ObjectMgr::LoadCreatureAddons()
{
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u creature addons in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
bool ObjectMgr::AddGOData(uint32 p_LowGuid, uint32 entry, uint32 mapId, float x, float y, float z, float o, uint32 spawntimedelay, float rotation0, float rotation1, float rotation2, float rotation3)
{
	...
	if (!map->Instanceable() && map->IsGridLoaded(x, y))
	{
		...
		{
			TC_LOG_ERROR("server.worldserver", "AddGOData: cannot add gameobject entry %u to map", entry);
			delete go;
			return false;
		}
	}

	TC_LOG_DEBUG("maps", "AddGOData: dbguid %u entry %u map %u x %f y %f z %f o %f", p_LowGuid, entry, mapId, x, y, z, o);
...
bool ObjectMgr::MoveCreData(uint32 guid, uint32 mapId, Position pos)
{
	...
	if (Map* map = sMapMgr->CreateBaseMap(mapId))
	{
		...
		if (!map->Instanceable() && map->IsGridLoaded(data.posX, data.posY))
		{
			...
			{
				TC_LOG_ERROR("server.worldserver", "AddCreature: cannot add creature entry %u to map", guid);
				delete creature;
				return false;
			}
		}
...
uint32 ObjectMgr::AddCreData(uint32 entry, uint32 /*team*/, uint32 mapId, float x, float y, float z, float o, uint32 spawntimedelay)
{
	...
	if (Map* map = sMapMgr->CreateBaseMap(mapId))
	{
		...
		if (!map->Instanceable() && !map->IsRemovalGrid(x, y))
		{
			...
			if (!creature->LoadCreatureFromDB(guid, map))
			{
				TC_LOG_ERROR("server.worldserver", "AddCreature: cannot add creature entry %u to map", entry);
				delete creature;
				return 0;
			}
		}
...
void ObjectMgr::LoadRealmCompletedChallenges()
{
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u realm completed challenges in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadChallengeRewards()
{
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u challenge mode rewards in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadItemTemplates()
{
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u item templates from Item-sparse.db2 in %u ms", sparseCount, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadItemSpecs()
{
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u item specs in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadItemSpecsOverride()
{
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u item specs override in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadAreaTriggerMoveTemplates()
{
	...
	if (!l_Result)
	{
		TC_LOG_INFO("server.loading", ">> Loaded 0 Areatrigger Move Template in %u ms", GetMSTimeDiffToNow(l_OldMSTime));
		return;
	}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u Areatrigger Move Template in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadAreaTriggerMoveSplines()
{
	...
	if (!l_Result)
	{
		TC_LOG_INFO("server.loading", ">> Loaded 0 Areatrigger Move Splines in %u ms", GetMSTimeDiffToNow(l_OldMSTime));
		return;
	}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u Areatrigger Move Splines in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadPetStatInfo()
{
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u pet stats definitions in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadPlayerInfo()
{
	...
	TC_LOG_INFO("server.loading", "Loading Player Create XP Data...");
	{
		...
		TC_LOG_INFO("server.loading", ">> Loaded %u xp for level definitions in %u ms", l_Count, GetMSTimeDiffToNow(oldMSTime));
	}
}
...
void ObjectMgr::GetPlayerClassLevelInfo(uint32 class_, uint8 level, uint32& baseHP, uint32& baseMana) const
{
	...
	if (!hp || !mp)
	{
		TC_LOG_ERROR("server.worldserver", "Tried to get non-existant Class-Level combination data for base hp/mp. Class %u Level %u", class_, level);
		return;
	}
...
void ObjectMgr::LoadQuests()
{
	...
	for (QuestMap::iterator iter = _questTemplates.begin(); iter != _questTemplates.end(); ++iter)
	{
		...
		if (qinfo->GetQuestMethod() >= 4)
			TC_LOG_ERROR("sql.sql", "Quest %u has `Method` = %u, expected values are 0, 1, 2 or 3.", qinfo->GetQuestId(), qinfo->GetQuestMethod());

		if (qinfo->IsAutoComplete() && qinfo->IsRepeatable())
		{
			TC_LOG_ERROR("sql.sql", "Quest %u is auto-complete and is repeatable", qinfo->GetQuestId());
...
void ObjectMgr::LoadInstanceEncounters()
{
	...
	do
	{
		...
		{
			TC_LOG_ERROR("sql.sql", "Table `instance_encounters` has an invalid encounter id %u, skipped!", l_EncounterID);
			continue;
		}

		if (l_LastEncounterDungeon && !sLFGDungeonStore.LookupEntry(l_LastEncounterDungeon))
		{
			TC_LOG_ERROR("sql.sql", "Table `instance_encounters` has an encounter %u (%s) marked as final for invalid dungeon id %u, skipped!", l_EncounterID, l_DungeonEncounter->NameLang, l_LastEncounterDungeon);
			continue;
		}
		...
		if (l_LastEncounterDungeon)
		{
			...
			{
				TC_LOG_ERROR("sql.sql", "Table `instance_encounters` specified encounter %u (%s) as last encounter but %u (%s) is already marked as one, skipped!", l_EncounterID, l_DungeonEncounter->NameLang, l_Itr->second->ID, l_Itr->second->NameLang);
				continue;
			}
		...
		case ENCOUNTER_CREDIT_KILL_CREATURE:
		{
			...
			if (!l_CreatureInfo)
			{
				TC_LOG_ERROR("sql.sql", "Table `instance_encounters` has an invalid creature (entry %u) linked to the encounter %u (%s), skipped!", l_CreditEntry, l_EncounterID, l_DungeonEncounter->NameLang);
				continue;
			}
		...
		case ENCOUNTER_CREDIT_CAST_SPELL:
			...
			{
				TC_LOG_ERROR("sql.sql", "Table `instance_encounters` has an invalid spell (entry %u) linked to the encounter %u (%s), skipped!", l_CreditEntry, l_EncounterID, l_DungeonEncounter->NameLang);
				continue;
			}
			break;
		default:
			TC_LOG_ERROR("sql.sql", "Table `instance_encounters` has an invalid credit type (%u) for encounter %u (%s), skipped!", l_CreditType, l_EncounterID, l_DungeonEncounter->NameLang);
			continue;
		}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u instance encounters in %u ms", l_Counter, GetMSTimeDiffToNow(l_OldMSTime));
}
...
WorldSafeLocsEntry const* ObjectMgr::GetClosestGraveYard(float x, float y, float z, uint32 MapId, uint32 team)
{
	...
	if (!zoneId)
	{
		if (z > -500)
		{
			TC_LOG_ERROR("server.worldserver", "ZoneId not found for map %u coords (%f, %f, %f)", MapId, x, y, z);
...
void ObjectMgr::LoadAccessRequirements()
{
	...
	do
	{
		...
		if (ar.item)
		{
			...
			if (!pProto)
			{
				TC_LOG_ERROR("server.worldserver", "Key item %u does not exist for map %u difficulty %u, removing key requirement.", ar.item, mapid, difficulty);
				ar.item = 0;
			}
		}
		...
		if (ar.item2)
		{
			...
			if (!pProto)
			{
				TC_LOG_ERROR("server.worldserver", "Second item %u does not exist for map %u difficulty %u, removing key requirement.", ar.item2, mapid, difficulty);
				ar.item2 = 0;
			}
		}
		...
		if (ar.leader_achievement)
		{
			...
			{
				TC_LOG_ERROR("sql.sql", "Required Leader achievement %u not exist for map %u difficulty %u, remove quest done requirement.", ar.leader_achievement, mapid, difficulty);
...
void ObjectMgr::LoadLFRAccessRequirements()
{
	...
	if (!l_Result)
	{
		TC_LOG_INFO("server.loading", ">> Loaded 0 LFR access requirement definitions. DB table `lfr_access_requirement` is empty.");
		return;
	}
	...
	do
	{
		...
		if (l_LFRAccessReq.Item)
		{
			...
			if (!l_ItemTemplate)
			{
				TC_LOG_ERROR("server.worldserver", "Key item %u does not exist for dungeon ID %u, removing key requirement.", l_LFRAccessReq.Item, l_DungeonID);
		...
		if (l_LFRAccessReq.Item2)
		{
			...
			if (!l_ItemTemplate)
			{
				TC_LOG_ERROR("server.worldserver", "Second item %u does not exist for dungeon ID %u, removing key requirement.", l_LFRAccessReq.Item2, l_DungeonID);
		...
		if (l_LFRAccessReq.QuestA)
		{
			...
			{
				TC_LOG_ERROR("sql.sql", "Required Alliance Quest %u not exist for dungeon ID %u, remove quest done requirement.", l_LFRAccessReq.QuestA, l_DungeonID);
		...
		if (l_LFRAccessReq.QuestH)
		{
			...
			{
				TC_LOG_ERROR("sql.sql", "Required Horde Quest %u not exist for dungeon ID %u, remove quest done requirement.", l_LFRAccessReq.QuestH, l_DungeonID);
		...
		if (l_LFRAccessReq.Achievement)
		{
			...
			{
				TC_LOG_ERROR("sql.sql", "Required Achievement %u not exist for dungeon ID %u, remove quest done requirement.", l_LFRAccessReq.Achievement, l_DungeonID);
		...
		if (l_LFRAccessReq.LeaderAchievement)
		{
			...
			{
				TC_LOG_ERROR("sql.sql", "Required Leader achievement %u not exist for dungeon ID %u, remove quest done requirement.", l_LFRAccessReq.LeaderAchievement, l_DungeonID);
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u lfr access requirement definitions in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
uint32 ObjectMgr::GenerateAuctionID()
{
	...
	{
		TC_LOG_ERROR("server.worldserver", "Auctions ids overflow!! Can't continue, shutting down server. ");
...
#endif /* CROSS */
	{
		TC_LOG_ERROR("server.worldserver", "EquipmentSet guid overflow!! Can't continue, shutting down server. ");
		World::StopNow(ERROR_EXIT_CODE);
	}
#ifndef CROSS
...
#endif /* CROSS */
	{
		TC_LOG_ERROR("server.worldserver", "Mail ids overflow!! Can't continue, shutting down server. ");
		World::StopNow(ERROR_EXIT_CODE);
	}
#ifndef CROSS
...
void ObjectMgr::LoadGarrisonPlotBuildingContent()
{
	...
	if (!l_Result)
	{
		TC_LOG_INFO("server.loading", ">> Loaded 0 garrison plot building content. DB table `garrison_plot_building_content` is empty.");
		return;
	}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u garrison plot building content in %u ms", l_Count, GetMSTimeDiffToNow(l_StartTime));
}
...
void ObjectMgr::LoadNpcRecipesConditions()
{
	...
	if (!l_Result)
	{
		TC_LOG_INFO("server.loading", ">> Loaded 0 npc recipe condition. DB table `npc_recipe_condition` is empty.");
		return;
	}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u npc recipe condition in %u ms", l_Count, GetMSTimeDiffToNow(l_StartTime));
}
...
#ifndef CROSS
	TC_LOG_INFO("server.loading", ">> Loaded the max pet number: %d in %u ms", m_PetNumber - 1, GetMSTimeDiffToNow(oldMSTime));
#endif /* not CROSS */
...
void ObjectMgr::LoadCorpses()
{
	...
	do
	{
		...
		{
			TC_LOG_ERROR("server.worldserver", "Corpse (guid: %u) have wrong corpse type (%u), not loading.", guid, type);
			continue;
		}
...
void ObjectMgr::LoadCurrencyOnKill()
{
	...
	do
	{
		...
		{
			TC_LOG_ERROR("sql.sql", "Table `creature_creature` have data for not existed creature entry (%u), skipped", l_Creature_id);
			continue;
		}
		...
		for (CurrencyOnKillEntry::const_iterator i = l_CurrOnKill.begin(); i != l_CurrOnKill.end(); ++i)
		{
			...
			{
				TC_LOG_ERROR("sql.sql", "CurrencyType (CurrencyTypes.dbc) %u does not exist but is used in `creature_currency`", i->first);
				continue;
			}
		}
...
void ObjectMgr::LoadPersonnalCurrencyOnKill()
{
	...
	if (!l_Result)
	{
		TC_LOG_ERROR("server.loading", ">> Loaded 0 personnal creature currency definitions. DB table `creature_loot_currency_personnal` is empty.");
		return;
	}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u personnal creature currency definitions in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadNPCSpellClickSpells()
{
	...
	for (uint32 l_Entry = 0; l_Entry < l_LastEntry; l_Entry++)
	{
		...
		{
			TC_LOG_ERROR("sql.sql", "npc_spellclick_spells: Creature template %u has UNIT_NPC_FLAG_SPELLCLICK but no data in spellclick table! Removing flag", l_CreatureTemplate->Entry);
...
void ObjectMgr::LoadCreatureQuestStarters()
{
	...
	for (QuestRelations::const_iterator l_Iterator = _creatureQuestRelations.begin(); l_Iterator != _creatureQuestRelations.end(); ++l_Iterator)
	{
		...
		if (!cInfo)
			TC_LOG_ERROR("sql.sql", "Table `creature_queststarter` have data for not existed creature entry (%u) and existed quest %u", l_Iterator->first, l_Iterator->second);
		else if (!(cInfo->NpcFlags1 & UNIT_NPC_FLAG_QUESTGIVER))
			TC_LOG_ERROR("sql.sql", "Table `creature_queststarter` has creature entry (%u) for quest %u, but npcflag does not include UNIT_NPC_FLAG_QUESTGIVER", l_Iterator->first, l_Iterator->second);
...
void ObjectMgr::LoadReservedPlayersNames()
{
	...
	do
	{
		...
		{
			TC_LOG_ERROR("server.worldserver", "Table `reserved_name` have invalid name: %s", name.c_str());
			continue;
		}
...
bool ObjectMgr::IsVendorItemValid(uint32 vendor_entry, uint32 id, int32 maxcount, uint32 incrtime, uint32 ExtendedCost, uint8 type, Player* player, std::set<uint32>* skip_vendors, uint32 ORnpcflag) const
{
	...
	{
		TC_LOG_ERROR("sql.sql", "Table `(game_event_)npc_vendor` have Item (Entry: %u, type: %u) with missing maxcount for vendor (%u), ignore", id, type, ExtendedCost, vendor_entry); ///<  Data argument not used by format string
		return false;
	}
	...
	for (uint32 l_Entry = 0; l_Entry < l_LastEntry; l_Entry++)
	{
		...
		for (uint16 lvl = l_CreatureTemplate->minlevel; lvl <= l_CreatureTemplate->maxlevel; ++lvl)
		{
			if (_creatureBaseStatsStore.find(MAKE_PAIR16(lvl, l_CreatureTemplate->unit_class)) == _creatureBaseStatsStore.end())
				TC_LOG_ERROR("sql.sql", "Missing base stats for creature class %u level %u", l_CreatureTemplate->unit_class, lvl);
		}
	}
...
void ObjectMgr::LoadCreatureGroupSizeStats()
{
	...
	if (!l_Result)
	{
		TC_LOG_INFO("server.loading", ">> Loaded 0 creature group size stats. DB table `creature_groupsizestats` is empty.");
		return;
	}
	...
	do
	{
		...
		{
			TC_LOG_ERROR("sql.sql", "Creature template entry (entry: %u) used in `creature_groupsizestats` does not exist.", l_CreatureEntry);
			continue;
		}
		...
		{
			TC_LOG_ERROR("sql.sql", "Group size %u (entry %u) used in `creature_groupsizestats` is invalid.", l_GroupSize, l_CreatureEntry);
			continue;
		}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u creature group size stats in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadFactionChangeQuests()
{
	...
	do
	{
		...
		{
			TC_LOG_ERROR("sql.sql", "Alliance quest %u referenced in `player_factionchange_quests` does not exist, pair skipped!", l_AllianceQuest);
			continue;
		}

		if (!sObjectMgr->GetQuestTemplate(l_HordeQuest))
		{
			TC_LOG_ERROR("sql.sql", "Horde quest %u referenced in `player_factionchange_quests` does not exist, pair skipped!", l_HordeQuest);
			continue;
		}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u faction change quest pairs in %u ms", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadHotfixData(bool p_Reload)
{
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u hotfix info entries in %u ms", l_Count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadResearchSiteZones()
{
	...
	TC_LOG_INFO("server.loading", ">> Loaded %lu Archeology research site zones in %u ms.", (unsigned long)sResearchSiteSet.size(), GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadResearchSiteLoot()
{
	...
	if (!result)
	{
		TC_LOG_INFO("server.loading", ">> Loaded 0 Archeology site loot. DB table `research_loot` is empty.");
		return;
	}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %lu Archeology research site loot in %u ms.", (unsigned long)_researchLoot.size(), GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadCharacterTemplateData()
{
	...
	if (!l_Result)
	{
		TC_LOG_INFO("server.loading", ">> Loaded 0 character templates.");
		return;
	}
	...
	do
	{
		...
		{
			TC_LOG_ERROR("sql.sql", "Template %u defined in `character_template` with class %u does not exists, skipped.", l_ID, l_CharacterTemplate->m_PlayerClass);
		...
		if (!l_ID)
		{
			TC_LOG_ERROR("sql.sql", "Template %u defined in `character_template` cannot have null index, skipped.", l_ID);
			delete l_CharacterTemplate;
			continue;
		}
		...
		if (l_ItemResult)
		{
			do
			{
				...
				if (!l_ItemTemplate)
				{
					TC_LOG_ERROR("sql.sql", "ItemID %u defined in `character_template_item` does not exist, ignoring.", l_TemplateItem.m_ItemID);
					continue;
				}
		...
		if (l_SpellsResult)
		{
			do
			{
				...
				{
					TC_LOG_ERROR("sql.sql", "SpellId %u defined in `character_template_spell` does not exist, ignoring.", l_SpellID);
					continue;
				}
		...
		if (l_ReputationResult)
		{
			do
			{
				...
				{
					TC_LOG_ERROR("sql.sql", "FactionID %u defined in `character_template_reputation` does not exist, ignoring.", l_FactionID);
					continue;
				}
	...
	while (l_Result->NextRow());
	TC_LOG_INFO("server.loading", ">> Loaded %u character templates %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadQuestObjectives()
{
	...
	if (!l_Result)
	{
		TC_LOG_INFO("server.loading", ">> Loaded 0 Quest Objectives. DB table `quest_objective` is empty.");
		return;
	}
	...
	do
	{
		...
		{
			TC_LOG_ERROR("sql.sql", "Quest Objective %u has non existant Quest Id %u! Skipping.", l_ObjectiveID, l_ObjectiveQuestId);
		...
		{
			TC_LOG_ERROR("sql.sql", "Quest Objective %u has invalid type %u! Skipping.", l_ObjectiveID, l_ObjectiveType);
			m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
			continue;
		}
		...
		case QUEST_OBJECTIVE_TYPE_NPC_INTERACT:
		{
			...
			{
				TC_LOG_ERROR("sql.sql", "Quest Objective %u has non existant Creature Id %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID);
				continue;
			}

			if (l_ObjectiveAmount <= 0)
			{
				TC_LOG_ERROR("sql.sql", "Quest Objective %u has valid Creature Id %u but amount %u is invalid! Skipping.", l_ObjectiveID, l_ObjectiveObjectID, l_ObjectiveAmount);
				m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
				continue;
			}
		...
		case QUEST_OBJECTIVE_TYPE_ITEM:
		{
			...
			{
				TC_LOG_ERROR("sql.sql", "Quest Objective %u has non existant Item Id %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID);
				m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
				continue;
			}
			...
			if (l_ObjectiveAmount <= 0)
			{
				TC_LOG_ERROR("sql.sql", "Quest Objective %u has valid Item Id %u but amount %u is invalid! Skipping.", l_ObjectiveID, l_ObjectiveObjectID, l_ObjectiveAmount);
				m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
				continue;
			}
		...
		case QUEST_OBJECTIVE_TYPE_GO:
		{
			...
			{
				TC_LOG_ERROR("sql.sql", "Quest Objective %u has non existant GameObject Id %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID);
				m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
				continue;
			}
			...
			if (l_ObjectiveAmount <= 0)
			{
				TC_LOG_ERROR("sql.sql", "Quest Objective %u has valid GameObject Id %u but amount %u is invalid! Skipping.", l_ObjectiveID, l_ObjectiveObjectID, l_ObjectiveAmount);
				m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
				continue;
			}
		...
		case QUEST_OBJECTIVE_TYPE_CURRENCY:
		{
			...
			{
				TC_LOG_ERROR("sql.sql", "Quest Objective %u has non existant Currency Id %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID);
				m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
				continue;
			}
		...
		case QUEST_OBJECTIVE_TYPE_SPELL:
		{
			...
			if (!l_Spell)
			{
				TC_LOG_ERROR("sql.sql", "Quest Objective %u has non existant Spell Id %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID);
				continue;
			}

			else if (!SpellMgr::IsSpellValid(l_Spell))
			{
				TC_LOG_ERROR("sql.sql", "Quest Objective %u has invalid Spell Id %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID);
				m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
				continue;
			}
			...
			if (l_ObjectiveAmount <= 0)
			{
				TC_LOG_ERROR("sql.sql", "Quest Objective %u has valid Spell Id %u but amount %u is invalid! Skipping.", l_ObjectiveID, l_ObjectiveObjectID, l_ObjectiveAmount);
				m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
				continue;
			}
		...
		case QUEST_OBJECTIVE_TYPE_FACTION_REP2:
		{
			...
			{
				TC_LOG_ERROR("sql.sql", "Quest Objective %u has non existant Faction Id %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID);
				m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
				continue;
			}
		...
		case QUEST_OBJECTIVE_TYPE_MONEY:
		{
			...
			{
				TC_LOG_ERROR("sql.sql", "Quest Objective %u is invalid, Quest %u already has the max amount of Quest Objective type %u! Skipping.", l_ObjectiveID, l_ObjectiveQuestId, l_ObjectiveType);
				m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
				continue;
			}
		...
		case QUEST_OBJECTIVE_TYPE_PET_BATTLE_ELITE:
		{
			...
			{
				TC_LOG_ERROR("sql.sql", "Quest Objective %u has non existant Battle Pet Species %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID);
				m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
				continue;
			}
		...
		case QUEST_OBJECTIVE_TYPE_PET_BATTLE_PVP:
		{
			...
			if (l_ObjectiveAmount <= 0)
			{
				TC_LOG_ERROR("sql.sql", "Quest Objective %u has invalid Pet Battle PvP win amount %u! Skipping.", l_ObjectiveID, l_ObjectiveObjectID, l_ObjectiveAmount); ///< Data argument not used by format string
				m_IgnoredQuestObjectives.push_back(l_ObjectiveID);
				continue;
			}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u Quest Objectives in %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadQuestObjectiveLocales()
{
	...
	if (!l_Result)
	{
		TC_LOG_INFO("server.loading", ">> Loaded 0 Quest Objective locale descriptions. DB table `locales_quest_objective` is empty.");
		return;
	}
	...
	do
	{
		...
		{
			TC_LOG_ERROR("sql.sql", "Quest Objective locale Id %u has invalid Quest Objective %u! Skipping.", l_Locale, l_ObjectiveID);
			continue;
		}

		if (l_Locale >= TOTAL_LOCALES)
		{
			TC_LOG_ERROR("sql.sql", "Locale Id %u for Quest Objective %u is invalid! Skipping.", l_Locale, l_ObjectiveID);
			continue;
		}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u Quest Objective visual effects in %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadItemBonusGroup()
{
	...
	if (!l_Result)
	{
		TC_LOG_INFO("server.loading", ">> Loaded 0 Item Bonus Group. DB table `item_bonus_group` is empty.");
		return;
	}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u item bonus group in %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadItemBonusGroupLinked()
{
	...
	if (!l_Result)
	{
		TC_LOG_INFO("server.loading", ">> Loaded 0 Item Bonus Group Linked. DB table `item_bonus_group_linked` is empty.");
		return;
	}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u item bonus group linked in %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadSpellInvalid()
{
	...
	if (!l_Result)
	{
		TC_LOG_INFO("server.loading", ">> Loaded 0 Spell Invalid. DB table `spell_invalid` is empty.");
		return;
	}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u Spell Invalid in %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadSpellStolen()
{
	...
	if (!l_Result)
	{
		TC_LOG_INFO("server.loading", ">> Loaded 0 Spell Stolen. DB table `spell_stolen` is empty.");
		return;
	}
	...
	do
	{
		...
		if (!spellInfo)
		{
			TC_LOG_ERROR("sql.sql", "SpellStolen: spell (spell_id:%d) does not exist in `Spell.dbc`.", l_SpellID);
			continue;
		}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u Spell Stolen in %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadDisabledEncounters()
{
	...
	if (!l_Result)
	{
		TC_LOG_INFO("server.loading", ">> Loaded 0 disabled ranking. DB table `instance_disabled_rankings` is empty.");
		return;
	}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u disabled ranking in %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}
...
void ObjectMgr::LoadConversationTemplates()
{
	...
	if (!l_Result)
	{
		TC_LOG_INFO("server.loading", ">> Loaded 0 conversation template. DB table `conversation_template` is empty.");
		return;
	}
	...
	do
	{
		...
		if (!l_LinesResult)
		{
			TC_LOG_ERROR("server.loading", "Conversation %u doesn't have any record in conversation_lines!", l_Conversation.Entry);
			continue;
		}
	...
	TC_LOG_INFO("server.loading", ">> Loaded %u conversation templates in %u ms.", l_Count, GetMSTimeDiffToNow(l_OldMSTime));
}