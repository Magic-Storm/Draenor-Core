...
WorldSocketMgr::StartReactiveIO (ACE_UINT16 port, const char* address)
{
    ...
    if (num_threads <= 0)
    {
        TC_LOG_ERROR("server.worldserver", "Network.Threads is wrong in your config file");
        return -1;
    }
    ...
    if (m_SockOutUBuff <= 0)
    {
        TC_LOG_ERROR("server.worldserver", "Network.OutUBuff is wrong in your config file");
        return -1;
    }
    ...
    {
        TC_LOG_ERROR("server.worldserver", "Failed to open acceptor, check if the port is free");
        return -1;
    }
...
WorldSocketMgr::OnSocketOpen (WorldSocket* sock)
{
    ...
    if (m_SockOutKBuff >= 0)
    {
        ...
        {
            TC_LOG_ERROR("server.worldserver", "WorldSocketMgr::OnSocketOpen set_option SO_SNDBUF");
            return -1;
        }
    }
    ...
    if (m_UseNoDelay)
    {
        if (sock->peer().set_option (ACE_IPPROTO_TCP,
            ...
            sizeof (int)) == -1)
        {
            TC_LOG_ERROR("server.worldserver", "WorldSocketMgr::OnSocketOpen: peer().set_option TCP_NODELAY errno = %s", ACE_OS::strerror (errno));
            return -1;
        }
    }
