...
bool AuthSession::_HandleLogonChallenge()
{
    ...
    TC_LOG_DEBUG("authserver", "[AuthChallenge] got full packet, %#04x bytes", challenge->size);
    TC_LOG_DEBUG("authserver", "[AuthChallenge] name(%d): '%s'", challenge->I_len, challenge->I);
    ...
    if (result)
    {
        ...
        TC_LOG_DEBUG("authserver", "'%s:%d' [AuthChallenge] Banned ip tries to login!", ipAddress.c_str(), port);
    }
    else
    {
        ...
        if (res2)
        {
            ...
            {
                TC_LOG_DEBUG("authserver", "[AuthChallenge] Account '%s' is locked to IP - '%s'", _login.c_str(), fields[4].GetCString());
                TC_LOG_DEBUG("authserver", "[AuthChallenge] Player address is '%s'", ipAddress.c_str());

                if (strcmp(fields[4].GetCString(), ipAddress.c_str()) != 0)
                {
                    TC_LOG_DEBUG("authserver", "[AuthChallenge] Account IP differs");
                    pkt << uint8(WOW_FAIL_LOCKED_ENFORCED);
                    locked = true;
                }
                else
                    TC_LOG_DEBUG("authserver", "[AuthChallenge] Account IP matches");
            }
            else
            {
                TC_LOG_DEBUG("authserver", "[AuthChallenge] Account '%s' is not locked to ip", _login.c_str());
                std::string accountCountry = fields[3].GetString();
                if (accountCountry.empty() || accountCountry == "00")
                    TC_LOG_DEBUG("authserver", "[AuthChallenge] Account '%s' is not locked to country", _login.c_str());
                else if (!accountCountry.empty())
                {
                    ...
                    if (PreparedQueryResult sessionCountryQuery = LoginDatabase.Query(stmt))
                    {
                        ...
                        TC_LOG_DEBUG("authserver", "[AuthChallenge] Account '%s' is locked to country: '%s' Player country is '%s'", _login.c_str(),
                            accountCountry.c_str(), loginCountry.c_str());

                        if (loginCountry != accountCountry)
                        {
                            TC_LOG_DEBUG("authserver", "[AuthChallenge] Account country differs.");
                            pkt << uint8(WOW_FAIL_UNLOCKABLE_LOCK);
                            locked = true;
                        }
                        else
                            TC_LOG_DEBUG("authserver", "[AuthChallenge] Account country matches");
                    }
                    else
                        TC_LOG_DEBUG("authserver", "[AuthChallenge] IP2NATION Table empty");*/
                }
            }
            ...
            if (!locked)
            {
                ...
                if (banresult)
                {
                    ...
                    {
                        pkt << uint8(WOW_FAIL_BANNED);
                        TC_LOG_DEBUG("authserver", "'%s:%d' [AuthChallenge] Banned account %s tried to login!", ipAddress.c_str(),
                            port, _login.c_str());
                    }
                    else
                    {
                        pkt << uint8(WOW_FAIL_SUSPENDED);
                        TC_LOG_DEBUG("authserver", "'%s:%d' [AuthChallenge] Temporarily banned account %s tried to login!",
                ...
                else
                {
                    ...
                    TC_LOG_DEBUG("authserver", "'%s:%d' [AuthChallenge] account %s is using '%c%c%c%c' locale (%u)",
                        ipAddress.c_str(), port, _login.c_str(),
...
bool AuthSession::_HandleLogonProof()
{
    ...
    TC_LOG_DEBUG("authserver", "Entering _HandleLogonProof");
    // Read the packet
    ...
    {
        TC_LOG_DEBUG("authserver", "'%s:%d' User '%s' successfully authenticated", GetRemoteIpAddress().c_str(), GetRemotePort(), _login.c_str());
    ...
    else
    {
        ...
        TC_LOG_DEBUG("authserver", "'%s:%d' [AuthChallenge] account %s tried to login with invalid password!",
            ...
            if (PreparedQueryResult loginfail = LoginDatabase.Query(stmt))
            {
                ...
                if (failed_logins >= MaxWrongPassCount)
                {
                    ...
                    if (WrongPassBanType)
                    {
                        ...
                        TC_LOG_DEBUG("authserver", "'%s:%d' [AuthChallenge] account %s got banned for '%u' seconds because it failed to authenticate '%u' times",
                    ...
                    else
                    {
                        ...
                        TC_LOG_DEBUG("authserver", "'%s:%d' [AuthChallenge] IP got banned for '%u' seconds because account %s failed to authenticate '%u' times",
...
{
    TC_LOG_DEBUG("authserver", "Entering _HandleReconnectChallenge");
    sAuthLogonChallenge_C *challenge = (sAuthLogonChallenge_C*)&_readBuffer;
    ...
    if (!result)
    {
        TC_LOG_ERROR("authserver", "'%s:%d' [ERROR] user %s tried to login and we cannot find his session key in the database.",
            GetRemoteIpAddress().c_str(), GetRemotePort(), _login.c_str());
        return false;
    }
...
{
    TC_LOG_DEBUG("authserver", "Entering _HandleReconnectProof");
    ...
    {
        TC_LOG_ERROR("authserver", "'%s:%d' [ERROR] user %s tried to login, but session is invalid.", GetRemoteIpAddress().c_str(),
...
{
    TC_LOG_DEBUG("authserver", "Entering _HandleRealmList");
    ...
    if (!result)
    {
        TC_LOG_ERROR("authserver", "'%s:%d' [ERROR] user %s tried to login but we cannot find him in the database.", GetRemoteIpAddress().c_str(),
            GetRemotePort(), _login.c_str());
        return false;
    }
